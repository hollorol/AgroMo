...
#' Bla
#' @param id id
#' @importFrom shiny NS tags checkboxInput selectInput textInput actionButton
#' plotOutput updateSelectInput observe imageOutput
...
agroMoParAnaUI <- function(id){
  ...
  tags$div(id = ns(id),
    ...
    tags$div(
      ...
      selectInput(ns("metfunc"),"metric function for calibration:",
                  choices=c("minima of RMSE"))),
    tags$div(id="paranatype","TYPE OF PARAMETER ANALYSIS:"),
    tags$div(
      id = paste0(ns("paranaradio"), "_container"), 
      radioButtons(ns("paranaradio"),"",
                   choices=c(" ", " ", " "), inline = FALSE)),
    ...
    tags$div(id=ns("cT"), checkboxInput(ns("copyThread"),
...
#' asdfasfd
#' @param input input
#' @importFrom shiny reactiveValues observe updateSelectInput observe
...
agroMoParAna <- function(input, output, session, baseDir){
  ...
  observe({
      updateSelectInput(session,"paranaini",
                        choices = list.dirs(file.path(baseDir(),"calibration"),
                                            full.names=FALSE, recursive=FALSE))
  })
  observe({
      updateSelectInput(session,"charfunc",
                        choices = list.files(file.path(baseDir(),"calibration",
                                                       input$paranaini),
                                             pattern="\\.set$"))
  })
  observe({
  ...
  observe({
      try({
         calFile <- list.files(file.path(baseDir(),"calibration",
                                         input$paranaini),
                               pattern="\\.cal$",full.names=TRUE)
         calInput$calFile <- calFile
         callines <- readLines(calFile)
         calInput$obsfile <- file.path(baseDir(),"calibration",
                                       isolate(input$paranaini),
                                       callines[1])
         ...
         calInput$setfile <- file.path(baseDir(),"calibration",
                                       isolate(input$paranaini), callines[2])
         updateSelectInput(session,"charfunc",
                           selected = basename(isolate(calInput$setfile)))
         calInput$constraints <- file.path(baseDir(),"calibration",
                                           isolate(input$paranaini),
                                           callines[3])
         updateSelectInput(session,"ctinfo",
  ...
  observeEvent(input$paranado,{
                   tryCatch({
                      ...
                      parameters <- read.csv(calInput$setfile,
                                             stringsAsFactors=FALSE, skip=1)
                      variableName <- read.table(calInput$setfile, nrows=1,
                                                 sep = ",",
                                                 stringsAsFactors = FALSE)
                      measurements <- read.table(measureFile,
                                                 stringsAsFactors = FALSE,
                                                 sep=";", header=TRUE)
                      calLines <- readLines(calFileName)
                      calLines <- calLines[setdiff(1:length(calLines),1:4)]
                      calTable <- read.csv2(textConnection(calLines),
                                            stringsAsFactors=FALSE)
                      if(nrow(calTable) == 0){
                          siteLine <- calLines
