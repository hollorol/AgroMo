...
variableMap <- new.env()
apply(data.frame(
  'longNames'= c("leaf dry matter","leaf dry matter in litter","fine root dry matter","yield","soft stem dry matter","projected lai","cum. evapotranspiration","cum_trans","rooting_depth","daily_gpp","daily_tr","daily_nee","tsoil_0","tsoil_1","tsoil_2","tsoil_3","tsoil_4","tsoil_5","tsoil_6","vwc_0","vwc_1","vwc_2","vwc_3","vwc_4","vwc_5","vwc_6","tsoil-profil","vwc-profil"),
  'shortNames'= c("leaf_DM", "leaflitr_DM", "froot_DM", "fruit_DM", "softstem_DM", "proj_lai", "cum_evap", "cum_trans", "rooting_depth", "daily_gpp", "daily_tr", "daily_nee", "tsoil_0", "tsoil_1", "tsoil_2", "tsoil_3", "tsoil_4", "tsoil_5", "tsoil_6", "vwc_0", "vwc_1", "vwc_2", "vwc_3", "vwc_4", "vwc_5", "vwc_6","tsoil-profil","vwc-profil")
),1,
      function(row){
...
apply(data.frame(
  longNames = c("leaf dry matter","leaf dry matter in litter","fine root dry matter","yield","soft stem dry matter","projected lai","cum. evapotranspiration","cum_trans","rooting_depth","daily_gpp","daily_tr","daily_nee","tsoil_0","tsoil_1","tsoil_2","tsoil_3","tsoil_4","tsoil_5","tsoil_6","vwc_0","vwc_1","vwc_2","vwc_3","vwc_4","vwc_5","vwc_6","tsoil-profil","vwc-profil"),
  convFactors = c(rep(1,3),10000,rep(1,24)),
...
multiPlot <- function(input, output, session, dataenv, measurement, outputNames, outputTable, experimentID, treatmentID,repetAvg = TRUE){
  ...
  simplePlots <- cbind.data.frame(outputTable(),sapply(outputTable()[,"variable"],function(variable){
  ...
  simplePlots[,"variable"] <- sapply(outputTable()[,"variable"],function(variable){
