...
setupGUI <- function(iniName,baseDir){
 ...
 searchBellow <- function(inFile, key, stringP = TRUE,  n=1, management = FALSE){
  ...
  ## browser()
  settings[["soil"]] <- basename(searchBellow(ini,"SOIL_FILE"))
  settings[["mgm"]] <- basename(searchBellow(ini,"MANAGEMENT_FILE"))
  ...
  ## mgm <- with(settings,{
  ##   if(mgm != "none"){
  ##     readLines(file.path(baseDir,"input","initialization","site",mgm))
  ##   } else {
  ##     NA
  ##   }
  ## })
